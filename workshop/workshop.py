...
def print_field(field):
    [print([get_cell_value(x) if x else 0 for x in row]) for row in field]
...
def is_win_condition(player, player_row, player_column, field):
    def normalize_player_position_in_direction(field, row, column, direction):
        ...
        column_max_boundary = len(field[0])

        while rows_min_boundary <= row < rows_max_boundary \
                and columns_min_boundary <= column < column_max_boundary:
    ...
    def is_win_condition_in_direction(field, initial_row, initial_column, direction):
        ...
        column_boundary = min(len(field[0]), column + 4 * column_delta)

        counter = 0

        is_row_included = rows_boundary == row
        is_column_included = column_boundary == column

        while (row != rows_boundary or is_row_included) \
                and (column != column_boundary or is_column_included) \
